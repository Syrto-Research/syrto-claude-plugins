# Project-context schema & Syrto Suite capability map

## Where context is written

Three paths relative to the active project root. Read before writing, merge, never
blind-overwrite. Do not create an index file and do not invent new paths. If there is no writable
project, keep the context in the current conversation and explain that it was not persisted.

### `.syrto/profile.md` — who the user is
...
### `.syrto/preferences.md` — how answers should look
...
### `.syrto/commercial-context.md` — the business
```markdown
---
...
- Fast lane — a quick, compact commercial profile of a company, from your calendar (the day's meetings, if a calendar connector is available; si programma come routine chiedendo «configura il briefing giornaliero») or spot by name, with the same fit read and ready talking points. *(briefing)*
