...
By now every buyer already carries a *provisional* tier from its L1 screen (Step 2/3). This step confirms it: the L2 deep-dive done on Tier 1/2, outside-the-box, and deep-dive names may promote or demote a buyer, and any Tier 2/3 borderline should have been taken to L2 before finalizing. Rationale is the single most important input here — every buyer, in every tier, needs a rationale that reflects the full framework from Step 2/3 (strategic fit, deal thesis, synergies, market position impact, integration logic, must-have vs. nice-to-have), confirmed financial capacity, and a clear ownership picture. A buyer without a grounded rationale doesn't belong on the list.
...
- Key decision maker (CEO, Corp Dev head, Partner) — informed by the UBO/ownership work in Step 2/3 (who actually has authority to decide)
...
Produce two deliverables with deliberately different levels of depth — the Excel is a scanning tool, the HTML is the narrative deliverable. **Every label, header, tier name and piece of rationale text in both deliverables must be written in Italian.** **Le deliverable HTML (buyer book e deep-dive) sono rese secondo `${CLAUDE_PLUGIN_ROOT}/shared/core.md`** (canone da `/preferences.md`, offerta del PDF, digest in chat): i contenuti e la struttura descritti qui sotto restano invariati, non costruire un HTML con stile proprio. L'Excel resta lo strumento di scanning, prodotto con la skill `xlsx`.
...
- For every buyer: the full rationale framework (all six elements), ownership structure summary (UBO, ownership type, listed Y/N — from Syrto for Italian entities), and financial capacity metrics (from Syrto for Italian entities) — this is what makes the HTML more complete than the Excel
- Enriched with additional Syrto data where useful (sector positioning, size comparison, benchmark vs. market via `syrto_get_spider_data`/`syrto_aggregate_companies`)
...
- Performance vs. the market: come crescita/profittabilità del buyer si confrontano con il benchmark di settore (dati spider/aggregate di Syrto)
...
- Ownership structure and UBO, and listed-company status (from Syrto for Italian entities)
