...
Rules: read before writing (in the Claude app's memory the read returns the version token writes
need); merge, never blind-overwrite; if a field a skill needs is absent, ask for it once in plain
language and offer to save it. A missing field is "not configured" — never invent a value.
