...
Riferimento della skill `market-sizing`. Leggi questo file prima di calibrare le soglie (Step 3) e prima di aggregare/deduplicare (Step 4–5). Le capacità Syrto citate qui (ricerca semantica, soglia di pertinenza, ordinamento per pertinenza, conteggio aggregato) sono mappate agli strumenti nella capability map (`{{ROOT}}/shared/syrto-reference.md`).
