...
description: >-
  ...
  companies only, found and sized in Syrto) and financial sponsors (from the bundled AIFI
  directory, Italian funds preferred), tiered on fit, ownership and financial capacity, with deliberate prominence for non-obvious
...
- **L0 - Discovery (near-free):** name and activity description, as company search returns them for strategics and as the AIFI lite table gives them for sponsors. Used to assemble the candidate pool and apply the fit gate. For strategics, put the size floor in the search itself: company search can require a minimum value of production (the target's own, for the reference year) and reports each match's value, so companies smaller than the target never enter the pool. A group shows up through its operating companies; the ownership structure at L2 shows the parent.
...
- Syrto is the source of record for every Italian company on the list: discovery, ownership and financials. The open web adds only (a) qualitative colour and strategic-intent signals on companies Syrto has already surfaced, (b) fund-level facts for financial sponsors beyond the AIFI list (current thesis, live check-size, recent deals), and (c) funds missing from that list (foreign funds with a real Italian footprint, non-AIFI funds). Where Syrto and a web source disagree, Syrto wins. If your client has no web access, mark the web-sourced fields "da verificare" rather than filling them from memory.
